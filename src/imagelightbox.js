...
(function (factory) {
	...
	$.fn.imageLightbox = function (opts) {
		var options = $.extend({
				...
				captionDataAttr: 	'title',
				captionFallbackAttr:'title',
				id:             'imagelightbox',
			...
			captionReset = function () {
				$captionObject.html('&nbsp;');
				if ($(target).data(options.captionDataAttr)) {
					$captionObject.html($(target).data(options.captionDataAttr));
				} else if ($(target).find('img').length > 0) {
					$captionObject.html($(target).find('img').attr(options.captionFallbackAttr));
				}
			},
